 ...
 * A Resolver Endpoint.
 */
export interface IResolverEndpoint extends IResource {
  /**
  ...
  /**
   * The ARN of the Resolver Endpoint.
   *
   * @attribute
   */
...
export interface IpAddress {
   ...
   * @default - Auto assign IP v4 address when use IP v4 address.
   */
  readonly ipv4?: string;
  /**
   * The IPv6 address that you want to forward DNS queries to.
   * @default - Auto assign IP v6 address when use IP v6 address.
   */
...
export interface IIpAddresses {
  bind(vpc: IVpc): IpAddressesConfig;
}
...
export class IpAddresses {
  static autoAssign(vpcSubnets?: SubnetSelection): IIpAddresses {
    return {
      bind: (vpc: IVpc) => ({
        ipAddresses: vpc.selectSubnets(vpcSubnets).subnets.map((subnet) => ({
          subnet,
        })),
      }),
    };
  }
  static specify(ipAddresses: IpAddress[]): IIpAddresses {
    return {
      bind: () => ({ ipAddresses }),
    };
  }
...
abstract class ResolverEndpointBase
  ...
  constructor(scope: Construct, id: string, props: ResolverEndpointBaseProps) {
    ...
    const resource = new CfnResolverEndpoint(this, 'Resource', {
      ...
      ipAddresses: ipAddressesConfig.ipAddresses.map((ipAddress) => ({
...
export interface IOutboundEndpoint extends IResolverEndpoint {
   ...
   */
  addRule(id: string, options: ForwardRuleOptions): ForwardRule;
}
...
export class OutboundEndpoint
  ...
  addRule(id: string, options: ForwardRuleOptions) {
    const rule = new ForwardRule(this, id, {
